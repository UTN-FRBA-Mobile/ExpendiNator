...
export const CategoryModel = {
    async getAllByUser(userId) {
        const [rows] = await pool.query(`SELECT c.id, c.name, c.color,
    ...
    async create(category) {
        ...
        try {
            ...
            const [result] = await conn.query("INSERT INTO categories (user_id, name, color) VALUES (?, ?, ?)", [category.user_id, category.name, category.color || null]);
    ...
    async findByName(userId, name) {
        const [rows] = await pool.query(`SELECT id, name, color FROM categories WHERE user_id = ? AND name = ? LIMIT 1`, [userId, name]);
        return rows[0] ?? null;
    },
    async findById(userId, id) {
        const [rows] = await pool.query(`SELECT id, name, color FROM categories WHERE user_id = ? AND id = ? LIMIT 1`, [userId, id]);
        return rows[0] ?? null;
    },
};
