...
export const BudgetModel = {
    async getAllByUser(userId) {
        const [rows] = await pool.query(`SELECT b.id, b.limit_amount, b.period, b.start_date, b.end_date,
              c.id AS category_id, c.name AS category_name, c.color AS category_color
         FROM budgets b
         JOIN categories c ON b.category_id = c.id
        WHERE b.user_id = ?
        ORDER BY b.start_date DESC, c.name ASC`, [userId]);
        return rows.map((r) => ({
            id: r.id,
    ...
    async create(b) {
        const [result] = await pool.query(`INSERT INTO budgets (user_id, category_id, limit_amount, period, start_date, end_date)
       VALUES (?, ?, ?, ?, ?, ?)`, [
            b.user_id,
            b.category_id,
            b.limit_amount,
        ...
        const [rows] = await pool.query(`SELECT b.id, b.limit_amount, b.period, b.start_date, b.end_date,
         ...
         FROM budgets b
         JOIN categories c ON b.category_id = c.id
        WHERE b.id = ? AND b.user_id = ?`, [id, b.user_id]);
        const r = rows[0];
        return {
            id: r.id,
            limit_amount: Number(r.limit_amount),
    ...
    async findByIdForUser(id, userId) {
        const [rows] = await pool.query(`SELECT id, user_id, category_id, limit_amount, period, start_date, end_date
         FROM budgets
        WHERE id = ? AND user_id = ?`, [id, userId]);
        return rows[0] || null;
    },
    async update(b) {
        const [result] = await pool.query(`UPDATE budgets
        ...
        const [rows] = await pool.query(`SELECT b.id, b.limit_amount, b.period, b.start_date, b.end_date,
         ...
         FROM budgets b
         JOIN categories c ON b.category_id = c.id
        WHERE b.id = ? AND b.user_id = ?`, [b.id, b.user_id]);
        const r = rows[0];
        return {
            id: r.id,
            limit_amount: Number(r.limit_amount),
    ...
    async getUsageByUser(userId, opts) {
    ...
    `;
        const params = [from, to, from, to, userId];
        if (activeOnly)
            params.push(from, to);
